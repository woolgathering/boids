...
Boids3D {
  ...
  *new {|numBoids = 10, timestep = 0.5, centerInstinct = 1, innerDistance = 1, matchVelocity = 1|
  ...
  prFillBoidList {|num|
    num.do{
      var boid;
      boid = BoidUnit3D.rand(bounds, centerInstinct, innerDistance, matchVelocity, gauss(workingMaxVelocity, workingMaxVelocity*0.1));
  ...
  maxVelocity_ {|val|
    ...
    boidList.do{|boid|
      boid.maxVelocity = gauss(workingMaxVelocity, workingMaxVelocity*0.1); // set it in each individual boid
    };
  }
  ...
  centerInstinct_ {|val|
    ...
    boidList.do{|boid|
      boid.instincts.add(\centerInstinct->gauss(val, val*0.1));
    };
  }
  ...
  innerDistance_ {|val|
    ...
    boidList.do{|boid|
      boid.instincts.add(\innerDistance->gauss(val, val*0.1));
    };
  }
...
BoidUnit3D {
  ...
  *rand {|bounds, centerOfMass, innerDistance, matchVelocity, maxVelocity = 5|
  ...
  init {|...args|
    bounds = bounds ? args[0] ? [[-500,500],[-500,500],[-500,500]]; // [ [xmin, xmax], [ymin, ymax]]
    vel = vel ? RealVector3D.newFrom(Array.fill(3, {rrand(0.0,3.0)}));
    pos = pos ? RealVector.rand(3,-1*bounds[0][0], bounds[0][0]).asRealVector3D;
    maxVelocity = maxVelocity ? args[4] ? 100; // max velocity

    // if these are not set, set them
    centerOfMass = args[2] ? RealVector.rand(3,-10,10).asRealVector3D;
