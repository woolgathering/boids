Automator {
  ...
  *new {|nodes, curve = \lin|
    ^super.newCopyArgs(nodes, curve).init;
  }
  ...
  *load {|path|
  ...
  show {
    ...
    // if the window is nil, it's the first time we've called show
    if (window.isNil)
      {
        name = name ?? {"Automator"};
        ...
        // else, the window is not nil so don't make a new one. Just get the old one.
        window.front;
      };
  }
