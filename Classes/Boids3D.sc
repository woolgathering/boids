...
Boids3D {
  ...
  *new {|numBoids = 10, timestep = 0.5, centerInstinct = 1, innerDistance = 1, matchVelocity = 1|
  ...
  prDoRules {
    var posSum = RealVector3D.zero, velSum = RealVector3D.zero, sizeOfNeighbors;
  ...
  prFillBoidList {|num|
    num.do{
      var boid;
      boid = BoidUnit3D.rand(bounds, centerInstinct, innerDistance, matchVelocity, minSpace, workingMaxVelocity, sd);
  ...
  addBoid {|initPos|
    ...
    boid = BoidUnit3D.new(initVel, initPos, bounds, centerInstinct, innerDistance, matchVelocity, minSpace, workingMaxVelocity, sd);
  ...
  bounds_ {|dim|
    ...
    if(dim.isArray) {
      ...
      "dim must be a three element array".error; // else something is wrong
      ^nil; // return nill
    };
  ...
  addTarget {|pos, gravity|
    ...
    targets.add(Dictionary.with(*[\pos->RealVector3D.newFrom(pos[..2]), \strength->gravity]));
  }
  ...
  editTarget {|index, pos, gravity|
    ...
    if(pos.notNil) {targets[index].add(\pos->RealVector3D.newFrom(pos[..2]))}; // should check here if target is a Vector or not
  ...
  addObstacle {|pos, repulsion|
    ...
    obstacles.add(Dictionary.with(*[\pos->RealVector3D.newFrom(pos[..2]), \strength->repulsion]));
  }
  ...
  minSpace_ {|val|
    minSpace = val;
    boidList.do(_.instincts.add(\minSpace->gauss(minSpace, minSpace*sd)));
  }
...
BoidUnit3D {
  ...
  *rand {|bounds, centerInstinct, innerDistance, matchVelocity, minSpace, maxVelocity, sd|
  ...
  init {|...args|
    ...
    pos = pos ? RealVector.rand(2,-1*bounds[0][0], bounds[0][0]).asRealVector3D;
    maxVelocity = maxVelocity ? gauss(args[5], args[5]*args[6]) ? 5; // max velocity

    // if these are not set, set them randomly
    centerOfMass = RealVector.rand(2,-10,10).asRealVector3D;
    innerDistance = RealVector.rand(2,-10,10).asRealVector3D;
    matchVelocity = RealVector.rand(2,-10,10).asRealVector3D;
  ...
  calcObstacles {|obstacles|
    var vec = RealVector3D.zero, distFromTarget, gravity, diff;
    ...
    switch (type)
      ...
      {\obstacle} {
        ...
        if(object.at(\strength).isArray) {
          ...
          // gravity = 3.collect{|i| this.prInverseSquare(diff[i], object.at(\strength)*1000).max(0)}; // check for distance in particular dimensions
        };
      };
    ^vec + ((diff/diff.norm)*gravity); // return
  }
